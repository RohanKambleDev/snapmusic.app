        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-8">
                ...
                @if (session('video_completed'))
                    ...
                    <div x-data="{ show: true }" x-show="show" class="relative rounded-2xl bg-emerald-500/10 border border-emerald-500/20 p-5 text-emerald-100 flex flex-col sm:flex-row items-center justify-between shadow-2xl backdrop-blur-md gap-4 max-w-3xl mx-auto">
                        <div class="flex items-center gap-4 w-full sm:w-auto">
                            ...
                            <div>
                                <p class="font-bold text-lg leading-tight text-white">SnapMusic Ready!</p>
                                <p class="text-emerald-300/60 text-sm mt-0.5">SnapMusic #{{ $data['id'] }} processed successfully.</p>
                            </div>
                        </div>
            ...
            <div class="bg-gray-900/50 backdrop-blur-xl border border-white/10 rounded-3xl p-4 sm:p-6 shadow-xl">
                <div class="flex flex-col lg:flex-row justify-between items-start lg:items-end gap-6">
                    <div>
                        ...
                        <p class="text-gray-400 text-sm">Manage, filter, and organize your generated SnapMusics.</p>
                    </div>
                        ...
                        <div class="video-card group relative bg-gray-900/40 backdrop-blur-md border border-white/5 rounded-2xl overflow-hidden hover:border-purple-500/30 hover:shadow-2xl hover:shadow-purple-500/10 transition-all duration-300 flex flex-col" data-job-id="{{ $job->id }}">
                            ...
                            <div class="p-4 flex-1 flex flex-col">
                                <div class="flex items-center justify-between mb-2">
                                    <h4 class="font-bold text-white text-sm truncate mr-2">SnapMusic #{{ $job->id }}</h4>
                                ...
                                <div class="mt-auto pt-4 flex items-center justify-between gap-2 border-t border-white/5">
                                    ...
                                    <form action="{{ route('make-a-video.destroy', $job) }}" method="POST" class="{{ $job->isCompleted() ? '' : 'w-full' }}" onsubmit="return confirm('Permanently delete this SnapMusic?');">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" aria-label="Delete SnapMusic" class="w-full flex items-center justify-center gap-1.5 py-2 rounded-lg bg-red-500/5 hover:bg-red-500/10 text-xs font-medium text-red-400 transition-colors border border-red-500/10 hover:border-red-500/20" title="Delete SnapMusic">
            ...
            @else
                ...
                <div class="flex flex-col items-center justify-center py-24 text-center">
                    ...
                    <h3 class="text-xl font-bold text-white mb-2">No SnapMusics found</h3>
                    <p class="text-gray-400 max-w-sm mx-auto mb-8">
                        @if(request()->anyFilled(['search', 'status', 'date']))
                            We couldn't find any SnapMusics matching your filters. Try adjusting your search criteria.
                        @else
                            Start your journey by creating your first SnapMusic. It's fast, easy, and free.
                        @endif
                    </p>
